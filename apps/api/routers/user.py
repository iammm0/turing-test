...
async def get_user_game_history(user_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def update_user(user_id: uuid.UUID, user_data: UserCreate, db: AsyncSession = Depends(get_db)):
